...
def main():
    writer = SummaryWriter('G:/PythonProject/src/Human_machine_interaction/multi_digits')
    config = Config()
    ...
    def val(net, criterion, eval_data_batch, epoch):
        print('Start val')
        ...
        writer.add_scalar('val_loss', loss_avg.val(), epoch)
    ...
    for epoch in range(config.nepoch):
        ...
        for batch_data in training_data_batch:
            ...
            if i % config.displayInterval == 0:
                ...
                writer.add_scalar('train_loss', loss_avg.val(), epoch * len(training_data_batch) + i)
                loss_avg.reset()
