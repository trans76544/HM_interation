...
class Config(object):
    def __init__(self):
        ...
        self.train_data = os.path.abspath(os.path.join(base_dir, "dataset/MNIST/mul-digits/train.txt"))
        self.eval_data = os.path.abspath(os.path.join(base_dir, "dataset/MNIST/mul-digits/eval.txt"))
        assert os.path.exists(self.train_data) and os.path.exists(self.eval_data)

        # 定义识别的标签类别
        self.label_classes = "0123456789"

        self.use_cuda = True

        self.device = torch.device("cuda" if torch.cuda.is_available() and self.use_cuda else "cpu")           # 检测是否存在GPU

        self.train_batch_size = 5                              # 训练batch_size       不定长输入时,这里的batch_size不适合过大
        self.eval_batch_size = 10                               # 校验batch_size

        self.img_height = 32            # 图像高度缩放至固定尺寸       注意设定时，改变下采样的次数
        self.img_width = 120            # 图像宽度缩放至固定尺寸
        ...
        self.lr = 0.0005                # learning rate for Critic, not used by adadealta
        ...
        self.nepoch = 20              # number of epochs to train for